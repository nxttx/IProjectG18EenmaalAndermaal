...
$search=$_POST['search'];
$searchInput = "%$search%";
$sql = "SELECT gebruikersnaam, emailadress  FROM gebruiker WHERE gebruikersnaam LIKE ? AND is_geverifieerd =?";
$stmt = $dbh->prepare($sql);
    $stmt->execute([$searchInput,$true]);
}
else{
                      ...
                      <form class='field' method='post'>
                        <tr>
                            ...
                            <td>                
                                ...
                                <button class='button is-primary' name='delete' type='submit' onclick='conf()'>Delete</button>
                            </td>
                         </tr>
                      </form>
";
...
$dbh = connectToDatabase();
      ...
      $sth5 = $dbh->prepare("DELETE w FROM bestand w inner join voorwerp v on voorwerp=voorwerpnummer where verkoper =:user");
        $sth5->bindParam(':user', $user);
        $sth5->execute();   
    
       $sth4 = $dbh->prepare("DELETE w FROM voorwerpinrubriek w inner join voorwerp v on voorwerp=voorwerpnummer where verkoper =:user");
        $sth4->bindParam(':user', $user);
        $sth4->execute();
    
    
    $sth3 = $dbh->prepare("DELETE FROM bod WHERE gebruiker =:user");
            $sth3->bindParam(':user', $user);
            $sth3->execute();
    
    $sth6 = $dbh->prepare("DELETE FROM voorwerp WHERE koper =:user");
            $sth6->bindParam(':user', $user);
            $sth6->execute();
    
       $sth2 = $dbh->prepare("DELETE FROM voorwerp WHERE verkoper =:user");
            $sth2->bindParam(':user', $user);
            $sth2->execute();
    
    $sth1 = $dbh->prepare("DELETE FROM verkoper WHERE gebruiker =:user");
            $sth1->bindParam(':user', $user);
            $sth1->execute();
 
    
    
    $sth = $dbh->prepare("DELETE FROM gebruiker WHERE gebruikersnaam =:user");
                        ...
                        <div class="container">
                            ...
                            <div class="card ">
                                <div class="card-content">
                                    <h2 class="title is-2  has-text-centered">Verwijder Gebruiker</h2>
                                </div>
                            </div>
                                                ...
                                                <div class="columns">
                                                    ...
                                                    <main class="column">
                                                        ...
                                                        <div class="columns is-centered">
															...
															<button class='button is-primary' name='sear' type='submit'>Search</button>
															</form>
